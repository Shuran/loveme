...
namespace Xamarin.Android.VideoTest.Opus
{
    public class Decoder
    {
        ...
        public byte[] Decode(byte[] encodedData)
        {
            ...
            try
            {
                return (byte[])JNIEnv.GetArray(OpusLibrary.Java_aopus_OpusLibrary_decoderDecode(JNIEnv.Handle, IntPtr.Zero, State, encodedDataPtr), JniHandleOwnership.DoNotTransfer, typeof(byte));
            }
            finally
            {
        ...
        public byte[] Decode(byte[] encodedData, bool fec)
        {
            ...
            try
            {
                return (byte[])JNIEnv.GetArray(OpusLibrary.Java_aopus_OpusLibrary_decoderDecode2(JNIEnv.Handle, IntPtr.Zero, State, encodedDataPtr, fec), JniHandleOwnership.DoNotTransfer, typeof(byte));
            }
            finally
            {
                JNIEnv.DeleteLocalRef(encodedDataPtr);
