...
using Xamarin.Mac.VP8;

[assembly: RequiredFramework("lib/Cocoa.VP8.dylib")]

namespace Xamarin.Mac.Conference.WebRTC.VP8
{
    /// <summary>
            ...
            if (_Encoder != null)
            {
                foreach (var packet in packets)
                {
                    ...
                    else if (packet is RTCPReportPacket)
                    {
                        ...
                        {
                            Log.InfoFormat("VP8 report: {0} packet loss ({1} cumulative packets lost)", block.PercentLost.ToString("P2"), block.CumulativeNumberOfPacketsLost.ToString());
