...
using Xamarin.Mac.Opus;

[assembly: RequiredFramework("lib/Cocoa.Opus.dylib")]

namespace Xamarin.Mac.Conference.WebRTC.Opus
{
    /// <summary>
    ...
    class OpusCodec : AudioCodec
    {
        ...
        {
            return _Padep.Packetize(encodedFrame, ClockRate, PacketTime);
        }
            ...
            if (_Encoder != null)
            {
                foreach (var packet in packets)
                {
                    if (packet is RTCPReportPacket)
                    {
                        ...
                        {
                            Log.InfoFormat("Opus report: {0} packet loss ({1} cumulative packets lost)", block.PercentLost.ToString("P2"), block.CumulativeNumberOfPacketsLost.ToString());
